...
export function Sidebar({
  ...
  const item = (
    m: { key: string; label: string; icon: string },
    disabled = false
  ) => {
      ...
      >
        <span
          style={{
            width: 16,
            textAlign: 'center',
            color: on ? colors.accent : 'inherit',
          }}
        >
          {m.icon}
        </span>
  ...
  return (
    ...
    >
      ...
      {isAdmin && (
        <>
          <div
            className="mt-4 mb-1 px-3 text-[10px] uppercase tracking-wider"
            style={{ color: colors.textFaint }}
          >
            관리
          </div>
      ...
      {/* 하단: 저장 모드 + 최신 조사일 */}
      <div
        className="mt-auto px-3 py-2 text-xs"
