// ────────────────────────────────────────────────────────────────
// 인스타그램 성과/신규 게시물 자동 수집 (Vercel Serverless Function)
//
// 호출 방법:
// - 수동 테스트: curl -H "Authorization: Bearer <CRON_SECRET>" https://.../api/collect-instagram
// - 자동 실행: vercel.json의 crons 설정 (운영 체크리스트에서 활성화 방법 안내)
//
// 이 파일은 인증 확인 → 동기화 실행 → sync_logs 기록 → 응답, 만 담당합니다.
// 실제 수집 로직은 api/_lib/instagram.ts에 있습니다.
// ────────────────────────────────────────────────────────────────
import type { VercelRequest, VercelResponse } from '@vercel/node';
import { getSupabaseAdmin } from './_lib/supabaseAdmin.js';
import { isAuthorized } from './_lib/auth.js';
import { startSyncLog, finishSyncLog } from './_lib/syncLog.js';
import { runInstagramSync } from './_lib/instagram.js';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
        const result = await runInstagramSync(client);
